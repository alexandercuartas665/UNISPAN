...
{
    public partial class RptSalesInvoiceTrackings : System.Web.UI.Page
    {
        ...
        private void RendeReport()
        {
            ...
            string apiurl = "api/ElectronicBilling/GetSalesInvoiceTrackings?guidfilter=" + filterobj;
            List<DTOSalesInvoiceTracking> result = Task.Run(async () => await Http.GetGenericAsync<List<DTOSalesInvoiceTracking>>(apiurl)).Result;
            ReportViewer1.LocalReport.DataSources.Add(new Microsoft.Reporting.WebForms.ReportDataSource("DataSet1", result));


            ReportViewer1.LocalReport.ReportEmbeddedResource = $"adesoft.adeposx.report.Reports.ElectronicBilling.RptSalesInvoiceTrackings{RptOpt}.rdlc";
