...
{
    public class LogisticsCoreService: ILogisticsCoreService
    {
        ...
        public static List<DTOOrderReportFilter> reportFilters = new List<DTOOrderReportFilter>();

        public LogisticsCoreService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor, ILogisticMasterDataService logisticMasterDataService)
        {
        ...
        public DTOOrderReportFilter GetReportFilter(string reportFilterId)
        {
            try
            {
                DTOOrderReportFilter filter = reportFilters.Where(x => x.GuidFilter == reportFilterId).FirstOrDefault();
                return filter;
            }
            catch (Exception ex)
            {
        ...
        public bool DeletePictures(DTOOrder dtoOrder)
        {
            try
            {
                ...
                var orderPictures = _dbcontext.OrderPictures
                    .Where(o => o.OrderId == dtoOrder.OrderId
                    && o.OrderType == dtoOrder.OrderType)
                    .ToList();

                if((orderPictures != null) && (orderPictures.Count() != 0))
                {
                    var picture = orderPictures.FirstOrDefault();
                    if (picture != null)
                    {
                        var path = Path.GetDirectoryName(picture.Path);
                        ...
                        _dbcontext.OrderPictures.RemoveRange(orderPictures);

                        this.CreateComment(dtoOrder, "Las imagenes fueron eliminadas.");
        ...
        public bool DeletePicture(DTOOrder dtoOrder, DTOOrderPicture dtoOrderPicture)
        {
            try
            {
                ...
                var orderPicture = _dbcontext.OrderPictures
                    .Where(o => o.OrderId == dtoOrderPicture.OrderId
                    && o.OrderType == dtoOrderPicture.OrderType
                    && o.Name == dtoOrderPicture.Name)
                    .FirstOrDefault();
                ...
                {                    
                    if (File.Exists(orderPicture.Path))
                    {
                        File.Delete(orderPicture.Path);
                    }

                    _dbcontext.OrderPictures.Remove(orderPicture);

                    this.CreateComment(dtoOrder, string.Format("La imagen {0} fue eliminada.", dtoOrderPicture.Name));
        ...
        public async Task UploadPicturesAsync(DTOOrder dtoOrder, IFileListEntry[] files)
        {
            try
            {
                ...
                {
                    var fileNamePath = string.Format("{0}/{1}", directory, file.Name);                                       
        ...
        public DTOOrder ChangeOrder(DTOOrder dtoOrdenOrig, DTOOrder dtoOrderNew)
        {
            try
            {
                ...
                var pictures = _dbcontext.OrderPictures
                    .Where(p => p.OrderType.Equals(dtoOrdenOrig.OrderType)
                    && p.OrderId.Equals(dtoOrdenOrig.OrderId))
                    .ToList();

                var imageStorePath = this._configuration.GetValue<string>("Logistics:ImageStorePath");
                var directory = string.Format("{0}/{1}", imageStorePath, dtoOrderNew.OrderId);
                if (!Directory.Exists(directory))
                {
                    Directory.CreateDirectory(directory);
                }
                ...
                {                                            
                    ...
                    {
                        var fileNamePath = string.Format("{0}/{1}", directory, System.IO.Path.GetFileName(picture.Path));
                        File.Copy(picture.Path, fileNamePath);                        

                        File.Delete(picture.Path);

                        picture.OrderId = dtoOrderNew.OrderId;
                        picture.Path = fileNamePath;
                    }
                }

                _dbcontext.UpdateRange(pictures);
        ...
        public List<DTOOrder> GetOrdersForChange(DateTime fromDate, DateTime toDate, DTOOrder dtoOrderDiff)
        {
            try
            {
                ...
                var dispatchs = _dbcontext.Orders
                    .Where(o => o.OrderType.Equals(OrderType.Dispatch)
                    && (o.DispatchDateTime.Date >= fromDate.Date && o.DispatchDateTime.Date <= toDate.Date)
                    && !o.Id.Equals(dtoOrderDiff.OrderId))
                    .ToList();
                ...
                dtoOrders = dispatchs
                    .Select(o => new DTOOrder()
                    {
                        OrderId = o.Id,
                        OrderNum = o.OrderNum,
                        Works = string.Format("{1} - {0}", o.Works, o.OrderType.Equals(OrderType.Dispatch) ? "DESPACHO" : "DEVOLUCIÓN")
                    })
                    .ToList();

                var returns = _dbcontext.Orders
                    .Where(o => o.OrderType.Equals(OrderType.Return)
                    && (o.ReturnDateTime.Date >= fromDate.Date && o.ReturnDateTime.Date <= toDate.Date)
                    && !o.Id.Equals(dtoOrderDiff.OrderId))
                    .ToList();

                var dtoOrders2 = returns
                    .Select(o => new DTOOrder()
                    {
                        OrderId = o.Id,
                        OrderNum = o.OrderNum,
